    ...
    $(function () {
        ...
        function bindAclModuleClick() {
            ...
            $(".aclModule-edit").click(function (e) {
                e.preventDefault();
                e.stopPropagation(); // 此处必须要取消冒泡,因为是个递归结构,冒泡的话会让一个点击被响应多个
                var aclModuleId = $(this).attr("data-id"); // 选中的部门id
                $("#dialog-aclModule-form").dialog({
                    modal: true,
                    title: "编辑权限模块",
                    open: function (event, ui) {
                    ...
                    buttons: {
                        "更新": function (e) {
                            e.preventDefault();
                            updateAclModule(false, function (data) { // success callback
                                $("#dialog-aclModule-form").dialog("close");
        ...
        function bindAclClick() {
            ...
            $(".acl-edit").click(function (e) {
                e.preventDefault();
                e.stopPropagation(); // 此处必须要取消冒泡,因为是个递归结构,冒泡的话会让一个点击被响应多个
                var aclId = $(this).attr("data-id"); // 选中的部门id
                $("#dialog-acl-form").dialog({
                    modal: true,
                    title: "编辑权限点",
                    open: function (event, ui) {
                    ...
                    buttons: {
                        "更新": function (e) {
                            e.preventDefault();
                            updateAcl(false, function (data) { // success callback
                                aclMap[aclId].aclModuleId = $("#aclModuleSelectId").val();
                                aclMap[aclId].status = $("#aclStatus").val();
                                aclMap[aclId].name = $("#aclName").val();
                                aclMap[aclId].type = $("#aclType").val();
                                aclMap[aclId].url = $("#aclUrl").val();
                                aclMap[aclId].seq = $("#aclSeq").val();
                                aclMap[aclId].remark = $("#aclRemark").val();
                                $("#dialog-acl-form").dialog("close");
                                showMessage("更新权限点", "更新成功,请刷新列表查看最新数据", true);
                            }, function (data) { // fail callback
